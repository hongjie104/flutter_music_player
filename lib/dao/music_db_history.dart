...
class HistoryDB {
  ...
  Future<int> addHistory(Map song) async {
    if ((await _getHistoryById(song['id']) != null)) {
      return _updateHistory(song);
    } else {
      var fav = {
        'id': song['id'],
        'name': song['name'],
  ...
  Future<Map<String, dynamic>?> _getHistoryById(var id) async {
    Database db = await MusicDB().getDB();
    List list = await db.query(table_name, where: 'id = ?', whereArgs: [id]);
    return list.length > 0 ? list[0] : null;
  }

  Future<int> _updateHistory(Map song) async {
    // 更新播放时间
    var fav = {
      'createTime': DateTime.now().millisecondsSinceEpoch,
    };
